...
public class OthelloGame implements Game {
	...
	this(player, in, ai, createFlag, verbose, 8);
    }
    ...
    public synchronized void computeScore() 
    {
		...
		final int outerRimValue = 10; 
		int cornerValue = board.length - 1;

		boolean DIEGOCALCULATION = true; 

		if(DIEGOCALCULATION)
		{
			// Handle all boundaries for home moves
			if(board[0][0] == 'X') { homeScore += outerRimValue * 2;}
			if(board[0][cornerValue] == 'X') { homeScore += outerRimValue; }
			if(board[cornerValue][0] == 'X') { homeScore += outerRimValue; }
			if(board[cornerValue][cornerValue] == 'X') { homeScore += outerRimValue * 2;}

			// Handle all boundaries for home moves
			if(board[0][0] == 'O') { homeScore += outerRimValue * 2; }
			if(board[0][cornerValue] == 'O') { homeScore += outerRimValue; }
			if(board[cornerValue][0] == 'O') { homeScore += outerRimValue; }
			if(board[cornerValue][cornerValue] == 'O') { homeScore += outerRimValue * 2;}
		}
		...
		for (int r = 0; r < board.length; r++)
		{
		    for (int c = 0; c < board[r].length; c++)
		    {
		    	if (board[r][c] == 'X') { homeScore++; }
				if (board[r][c] == 'O') { awayScore++; }
			}
		}
    }
