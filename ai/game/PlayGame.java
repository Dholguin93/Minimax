...
public class PlayGame {
	...
	case TTT:
	    ...
	    case 2: 
	    case 3: 
	    case 4: ai[0] = new TicTacToeAIMinimax(); break;  // "Perfect" play
	    }
	    switch (awayAI) {
	...
	case OTHELLO_MINI:
	case OTHELLO:
	    if (homeAI > 0) ai[0] = new OthelloAI(); else ai[0] = null;
	    if (awayAI > 0) ai[1] = new OthelloAI(); else ai[1] = null;
	    break;
	}
    }
	...
	case NIM:
	    ...
	    break;
	case OTHELLO_MICRO: dim -= 2;  // Make sure dim becomes 4
	case OTHELLO_MINI:  dim -= 2;  // Make sure dim becomes 6
	case OTHELLO: 
	    for (int p = 0; p < 2; p++)
	...
	int awayAI = 1;  // ... same ...
	GameType gameType = GameType.OTHELLO_MINI;  // Default is OTHELLO_MINI
	...
	for (String arg: args) {
		...
		case "--home": 
		    switch (params[1]) {
		    case "ai": 
		    case "1": homeAI = 1; break;
		    case "2": homeAI = 2; break;
		    case "3": homeAI = 3; break;
		    case "4": homeAI = 4; break;
		    case "human": 
		    case "0": homeAI = 0; break;
		...
		case "--away": 
		    switch (params[1]) {
		    case "ai": 
		    case "1": awayAI = 1; break;
		    case "2": awayAI = 2; break;
		    case "3": awayAI = 3; break;
		    case "4": awayAI = 4; break;
		    case "human": 
		    case "0": awayAI = 0; break;
	...
	System.err.println("         --game=XXX            -- Can be either NIM, TTT, OTHELLO_MICRO, OTHELLO_MINI, OTHELLO (default OTHELLO_MINI).");
